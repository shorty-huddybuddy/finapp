...
// var client *firestore.Client = database.GetFirestoreClient()
...
func FetchEvents(c *fiber.Ctx) error {
	...
	ref := database.FirebaseDB.NewRef("calendar_events")

	println("in fetching events")
...
// CreateEvent - Adds an event to Firebase Realtime Database
func CreateEvent(c *fiber.Ctx) error {
	...
	// Reference the "calendar_events" node in Realtime DB
	ref := database.FirebaseDB.NewRef("calendar_events")
	// print(ref)
...
// Update an event in Firebase Realtime Database
func UpdateEvent(c *fiber.Ctx) error {
	...
	// Reference the event by its unique ID in Firebase RTDB
	ref := database.FirebaseDB.NewRef("calendar_events").Child(id)
...
func FetchNotifications(c *fiber.Ctx) error {
	...
	println("in notification events ")
	...
	for _, event := range eventsMap {
		...
		// println(event.Start[:10])
		// println(eventTime.String())
...
func FetchMarketEvents(c *fiber.Ctx) error {
	marketEvents := []string{"IPO: TechCorp launching next week", "Q2 Results: Major banks reporting this month"}
	return c.JSON(marketEvents)
}

// Fetch risk alerts
func FetchRiskAlerts(c *fiber.Ctx) error {
	riskAlerts := []string{"High market volatility expected next week", "Currency fluctuations may impact international investments"}
